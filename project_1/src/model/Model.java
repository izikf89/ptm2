...
public class Model extends Observable implements Observer {
    ...
    public void getRoute(int routhX, int routhY, int getX, int getY, int[][] chart) {
        this.planeX = routhX;
        this.planeY = routhY;
        this.markX = getX;
        this.markY = getY;
    ...
    private void doRoute(String[] solution) {
        ...
        for(int i=0;i<intersections.size();i++) {
            ...
            int degree = getLevel(direct);
            if((tmp <= 15) && (tmp > 5)) {
                if(i+1<intersections.size())
                {
                    if(degree!=360 && degree!=90) {
                        if (degree < getLevel(intersections.get(i + 1)[0]))
                            degree += 45;
                        else
                            degree -= 45;
                    }
                    else if(degree == 360){
                        if(getLevel(intersections.get(i + 1)[0]) == 90)
                            degree = 45;
                        else
                            degree -= 45;
                    }
                    ...
                    {
                        if(getLevel(intersections.get(i + 1)[0]) == 360)
                            degree = 45;
                        else
                            degree -= 45;
                    }
    ...
    public Model() {
        ...
        rudder = new Thread(() -> {this.rs();});
    }

    public void execute() { interpter.execute(); }

    public void closeAP() { interpter.stop(); }
    ...
    public void route(double startX,double startY, double offset){
        this.offset = offset;
        this.startX = startX;
        this.startY = startY;
        new Thread(() -> {
    ...
    public double up(double currentHeading) {
    ...
    public void getp(String ip,int port) {
        try {
    ...
    public double down(double currentHeading) {
        int tmp = (int)currentHeading - 7;
